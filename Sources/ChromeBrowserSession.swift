...
final class ChromeBrowserSession: BrowserSession, @unchecked Sendable {
    ...
    /// read stream URLs directly out of API/XHR JSON responses (e.g. `…/api/match/detail?stream=true`)
    /// when an anti-bot player receives its stream config over the network but never issues the media
    /// request itself — so there is no `.m3u8` request to capture, only the JSON that describes it.
    ...
    private func configure(profile: BrowserProfile) async throws {
        ...
        // enables Network (so the parent target's capture — which already works on sites like
        // streamed.st — is unchanged), extends auto-attach to child targets, and installs the stealth
        // script and UA override.
